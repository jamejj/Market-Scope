...
def with_signal_display_columns(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    output["Ocena kierunku"] = output.apply(display_radar_direction, axis=1)
...
def build_signal_radar_brief(
    ...
    confirmed = confirmed_ml_long_rows(frame)
    priority = frame[
    ...
    thesis = display_radar_thesis(signal_brief_value(leader, "Teza radaru"))
    leader_mode = signal_brief_value(leader, "Tryb analizy")
    leader_state = dataframe_machine_decision_state(leader)
    probability = value_pct(
        finite_probability(leader.get("P(wzrost)"))
        if leader_mode == "ML" and "P(wzrost)" in leader
        else None
    )
    expected = signed_pct(leader.get("Oczekiwany ruch") if "Oczekiwany ruch" in leader else None)
    quality = display_model_quality(signal_brief_value(leader, "Jakość modelu"))
    ...
    risk_symbols = _unique_symbols(radar_risk_rows(frame))
    risk_value = f"{risk_symbols} alertów" if risk_symbols else "brak alertów ryzyka"
    ...
    elif leader_mode == "ML" and leader_state == MachineDecisionState.SHORT:
        headline = f"{symbol} jest najwyżej ocenianym potwierdzonym setupem spadkowym ML."
        body = (
            f"Horyzont {horizon}, P(wzrost) {probability}, oczekiwany ruch {expected}. "
            f"Teza systemu: {thesis}. To alert do ręcznej analizy ryzyka, nie automatyczna rekomendacja sprzedaży."
        )
    elif leader_mode == "ML" and leader_state == MachineDecisionState.NEUTRAL:
        headline = f"{symbol} jest najwyżej ocenianą obserwacją ML, ale bez potwierdzenia kierunku."
        body = (
            f"System oznaczył go jako {action} na horyzoncie {horizon}. "
            f"P(wzrost) {probability}, oczekiwany ruch {expected}; teza: {thesis}."
        )
    elif leader_mode == "ML":
        headline = f"{symbol} jest wysoko w radarze, ale jego klasyfikacja ML nie jest wiarygodna."
        body = (
            f"System oznaczył go jako {action} na horyzoncie {horizon}. "
            f"Teza: {thesis}. Rekord wymaga odświeżenia zamiast interpretacji kierunkowej."
        )
...
def setup_risk_note(row: pd.Series) -> tuple[str, str]:
    ...
    quality = display_model_quality(signal_brief_value(row, "Jakość modelu"))
    ...
    if quality.upper().startswith("NISKA"):
        notes.append("bieżąca ocena modelu jest niska")
    elif "FAST" in quality.upper():
        notes.append("brak pełnej oceny ML")
...
def build_setup_drilldown(row: pd.Series) -> dict:
    ...
    thesis = display_radar_thesis(signal_brief_value(row, "Teza radaru"))
    grade = signal_brief_value(row, "Setup grade")
    direction = display_radar_direction(row)
    ...
    machine_state = dataframe_machine_decision_state(row)

    if machine_state == MachineDecisionState.LONG:
        headline = f"{symbol} ma potwierdzony setup wzrostowy ML na horyzoncie {horizon}."
        body = (
    ...
    elif machine_state == MachineDecisionState.SHORT:
        headline = f"{symbol} ma potwierdzony setup spadkowy ML na horyzoncie {horizon}."
        body = (
            f"Radar widzi {action.lower()} i modelowy kierunek: {direction}. "
            f"Teza: {thesis}. To jest alert do pełnej ręcznej analizy ryzyka, nie automatyczna rekomendacja sprzedaży."
        )
    elif machine_state == MachineDecisionState.NEUTRAL:
        headline = f"{symbol} ma policzony ML, ale bez potwierdzenia kierunku."
        body = (
            f"Model został uruchomiony, lecz wspólny kontrakt nie potwierdził kierunku. "
            f"Teza radaru: {thesis}. Najrozsądniej traktować to jako obserwację, nie sygnał."
        )
    elif mode == "ML":
        headline = f"{symbol} ma brak wiarygodnej klasyfikacji ML."
        body = (
            f"Rekord nie spełnia kontraktu decyzji maszyny. Teza radaru: {thesis}. "
            "Nie należy interpretować go kierunkowo do czasu odświeżenia danych."
        )
    else:
...
def render_setup_cockpit(frame: pd.DataFrame, snapshot: dict | None = None) -> None:
    ...
    drilldown = build_setup_drilldown(row)
    cards = "".join(
...
def render_start_guidance(guidance: dict, snapshot: dict, cockpit: dict | None) -> None:
    ...
    if panel == "analysis" and saved and saved.get("years") == years:
        ...
        if records.empty:
            ...
            st.dataframe(
                radar_display_frame(records.head(10), columns=preferred),
...
def render_start_dashboard(
    *,
    ...
    with st.expander("Jak czytać prognozę?"):
        ...
        - **AUC** — {AUC_DIRECTION_HELP.removeprefix("AUC ")}
...
def render_evidence_status(result: dict, report: dict) -> None:
    ...
    try:
        ...
        st.warning(
            f"**Status zarejestrowanych dowodów przewagi dla {horizon_label}**\n\n"
            "Status dowodów jest chwilowo niedostępny — kontrola integralności rejestru dowodów nie przeszła."
        )
        return
    st.info(
        f"**Status zarejestrowanych dowodów przewagi dla {horizon_label}**\n\n"
    ...
    with st.expander("Szczegóły dowodów"):
        ...
        details = [
            ...
            f"- Zakres dowodów: `{assessment.evidence_scope.value if assessment.evidence_scope else 'BRAK ZAREJESTROWANEGO CLAIMU'}`",
            f"- Rejestr: `{assessment.provenance.get('registry_id') or '—'}`",
        ]
        if historical_claim:
            details.extend([
                f"- Claim historyczny: `{historical_claim.get('claim_id') or '—'}`",
                f"- Protokół historyczny: `{historical_claim.get('protocol_id') or '—'}`",
                f"- Dowody zaktualizowane: `{historical_claim.get('evidence_updated_at') or '—'}`",
            ])
...
def render_analysis(
    ...
    *,
    ...
    with st.expander(MODEL_DIAGNOSTIC_TITLE):
        diagnostic_rows = []
        for horizon, forecast in result["forecasts"].items():
            diagnostic_rows.append({
                ...
                ACCURACY_BASELINE_DELTA_LABEL: forecast["accuracy"] - forecast["baseline_accuracy"],
        ...
        st.dataframe(
            diagnostics.style.format({
                ...
                "Trafność prostego bazowego": "{:.1%}", ACCURACY_BASELINE_DELTA_LABEL: "{:+.1%}",
...
def _render_ranking_table(frame: pd.DataFrame, title: str, empty_text: str) -> None:
    ...
    formats = {
        ...
        "Risk/reward": "{:.2f}", "Edge score": "{:.2f}", "Score potencjału": "{:.2f}",
        "Deep score": "{:.0f}",
    ...
    display_frame = radar_display_frame(frame, columns=columns)
...
def render_signal_journal() -> None:
    ...
    with tabs[0]:
        if closed.empty:
            ...
            lab_cols[0].subheader(PAPER_RESULTS_BY_CLASS_TITLE)
            lab_cols[0].dataframe(
                by_class.style.format({"Trafność": "{:.1%}", "Średni_wynik": "{:+.1%}", "Mediana": "{:+.1%}"}),
                use_container_width=True, hide_index=True,
            )
            lab_cols[1].subheader(PAPER_RESULTS_BY_DIRECTION_TITLE)
...
def render_signal_dashboard() -> None:
    ...
    risk_rows = radar_risk_rows(frame)
    ...
    risk_label = "brak" if _unique_symbols(risk_rows) == 0 else str(_unique_symbols(risk_rows))
    ...
    radar_brief = build_signal_radar_brief(frame, snapshot)
    radar_cards = "".join(
    ...
    with horizon_tabs[0]:
        ...
        render_setup_cockpit(base, snapshot)
        hot_pool = base.copy()
        ...
        priority_display = radar_display_frame(priority, columns=compact_columns)
        st.dataframe(priority_display.style.format(formats, na_rep="—"), use_container_width=True, hide_index=True)
        st.markdown("#### 2. Autentyczne hot movers — wybrane innym kryterium")
        hot_display = radar_display_frame(hot_now, columns=compact_columns)
        st.dataframe(hot_display.style.format(formats, na_rep="—"), use_container_width=True, hide_index=True)
        ...
        if risk_now.empty:
            ...
            risk_display = radar_display_frame(risk_now, columns=compact_columns)
            st.dataframe(risk_display.style.format(formats, na_rep="—"), use_container_width=True, hide_index=True)
    with horizon_tabs[1]:
        st.subheader("Setup intelligence")
        st.caption("Rozbicie score na elementy, które trader sprawdza ręcznie: impet, trend, kontrolę ryzyka, płynność i bieżące wsparcie ML.")
        setup_frame = (
        ...
        setup_display = radar_display_frame(setup_frame, columns=setup_columns)
        st.dataframe(setup_display.style.format(formats, na_rep="—"), use_container_width=True, hide_index=True)
        st.info(
    ...
    with horizon_tabs[3]:
        ...
        rr_display = radar_display_frame(rr, columns=rr_columns)
        st.dataframe(rr_display.style.format(formats, na_rep="—"), use_container_width=True, hide_index=True)
    for tab, horizon, title in [
        ...
        with tab:
            ...
            candidates = confirmed_ml_long_rows(scoped)
            if candidates.empty:
...
with lab_model_tab:
    ...
    s3.markdown("<div class='pro-card'><h3>3. Radar</h3><p>Liczy momentum, setup intelligence, risk/reward i Score potencjału, żeby ustalić priorytet dalszej analizy.</p></div>", unsafe_allow_html=True)
    ...
    with st.expander("Znaczenie parametrów i skrótów"):
        ...
        - **Score potencjału** — priorytet analizy łączący oczekiwany ruch, P(wzrost), AUC/Brier, trend i zmienność.
        - **Setup score** — ocena spójności układu: momentum, trend, kontrola ryzyka, płynność i bieżące wsparcie ML.
        ...
        - **Deep score** — priorytet do pełnego ML, łączący Setup score, momentum, Score potencjału i kontrolę ryzyka.
